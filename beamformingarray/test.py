...
class Beamformer:
    def __init__(self,n_channels=8,spacing=0.03,sample_rate=48000):
        ...
        self.last_channel_shifts=np.zeros(n_channels)
    ...
    def calculate_channel_shift(self,samples):
        ...
        self.shift_to_angle(channel_shifts)
        # print(np.average)
    ...
    def shift_to_angle(self,channel_shifts):
        self.delays=channel_shifts*self.sample_dur
        ang=np.zeros(self.n_channels)
        for i in range(self.n_channels):
            ang[i]=np.degrees(np.arccos(v*self.delays[i]/i/self.spacing/(10**6)))%360
        print(ang)
...
writer.write("./beamformingarray/output5_100res8.wav",1*48000)   
# print(res)
# beam = Beamformer(4)
